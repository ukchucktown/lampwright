...
`lampwright` will constrain every process invocation it constructs to a
...
`npx` acquisition runs from Lampwright-owned local state with an isolated npm
cache, never from the user's project, and uses the exact approved package
version. Direct and ephemeral Owner invocations receive Lampwright-owned
...
trusted Owner lifecycle operation, not a claim that Lampwright can mediate
...
This limitation does not weaken Lampwright-owned mutations. Quarantine and record
