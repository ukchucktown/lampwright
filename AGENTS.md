...
- Do not cap lower-effort Executor invocations. The Advisor may continue issuing targeted correction or verification calls without repeated user approval while the issue remains in scope. Keep at most one Executor active at a time, and replace an exhausted or stalled Executor when necessary to complete the accepted issue.
